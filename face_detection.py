import numpy as np
import cv2
from operator import itemgetter
caffe_root = '/Users/lixile/caffe/'
import sys
sys.path.insert(0, caffe_root + 'python')
import caffe

def find_initial_scale(net_kind, small_face_size):
    return  float(small_face_size) / net_kind

def resize_image(caffe_img_resized, scale_factor):
    height, width, channels = caffe_img_resized.shape
    new_height = int(height / scale_factor)
    new_width = int(width / scale_factor)
    new_dim = (new_width, new_height)
    img_resized = cv2.resize(caffe_img_resized, new_dim)
    return img_resized


def draw_rectangle(net_kind, img, face):
    x = face[0]
    y = face[1]
    scale = face[2]
    original_x = int(x * scale)
    original_y = int(y * scale)
    original_x_br = int(x * scale + net_kind * scale)
    original_y_br = int(y * scale + net_kind * scale)
    cv2.rectangle(img, (original_x, original_y), (original_x_br, original_y_br), (255, 0, 0), 2)

def IoU(rect_1, rect_2):
    x11 = rect_1[0]
    y11 = rect_1[1]
    x12 = rect_1[2]
    y12 = rect_1[3]  # first rectangle bottom right y
    x21 = rect_2[0]  # second rectangle top left x
    y21 = rect_2[1]  # second rectangle top left y
    x22 = rect_2[2]  # second rectangle bottom right x
    y22 = rect_2[3]  # second rectangle bottom right y
    x_overlap = max(0, min(x12, x22) -max(x11, x21))
    y_overlap = max(0, min(y12, y22) -max(y11, y21))
    intersection = x_overlap * y_overlap
    union = (x12 - x11) * (y12 - y11) + (x22 - x21) * (y22 - y21) - intersection
    return float(intersection) / union

def IoM(rect_1, rect_2):
    x11 = rect_1[0]  # first rectangle top left x
    y11 = rect_1[1]  # first rectangle top left y
    x12 = rect_1[2]  # first rectangle bottom right x
    y12 = rect_1[3]  # first rectangle bottom right y
    x21 = rect_2[0]  # second rectangle top left x
    y21 = rect_2[1]  # second rectangle top left y
    x22 = rect_2[2]  # second rectangle bottom right x
    y22 = rect_2[3]  # second rectangle bottom right y
    x_overlap = max(0, min(x12, x22) - max(x11, x21))
    y_overlap = max(0, min(y12, y22) - max(y11, y21))
    intersection = x_overlap * y_overlap
    rect_1_area = (y12 - y11) * (x12 -x11)
    rect_2_area = (y22 - y21) * (x22 - x21)
    min_area = min(rect_1_area, rect_2_area)
    return float(intersection) / min_area


def localNMS(rectangles):
    result_rectangles = rectangles[:]
    number_of_rects = len(result_rectangles)
    threshold = 0.3
    cur_rect = 0
    while cur_rect < number_of_rects - 1:
        rects_to_compare = number_of_rects - cur_rect - 1
        cur_rect_to_compare = cur_rect + 1
        while rects_to_compare > 0:
            if(IoU(result_rectangles[cur_rect], result_rectangles[cur_rect_to_compare]) >= threshold) \
                    and (result_rectangles[5] == result_rectangles[cur_rect_to_compare][5]):
                del result_rectangles[cur_rect_to_compare]
                number_of_rects -= 1
            else:
                cur_rect_to_compare += 1
            rects_to_compare -= 1
        cur_rect += 1
    return result_rectangles


def globalNMS(rectangles):
    result_rectangles = rectangles[:]
    number_of_rects = len(result_rectangles)
    threshold = 0.3
    cur_rect = 0
    while cur_rect < number_of_rects - 1:
        rects_to_compare = number_of_rects - cur_rect- 1
        cur_rect_to_compare = cur_rect + 1
        while rects_to_compare > 0:
            if IoU(result_rectangles[cur_rect], result_rectangles[cur_rect_to_compare]) >= 0.2 \
                    or ((IoM(result_rectangles[cur_rect], result_rectangles[cur_rect_to_compare]) >= threshold)
                        and (result_rectangles[cur_rect_to_compare][5] < 0.85)):
                del result_rectangles[cur_rect_to_compare]
                number_of_rects -= 1
            else:
                cur_rect_to_compare += 1
            rects_to_compare -= 1
        cur_rect += 1
    return result_rectangles


def globalNMS_specify(rectangles):
    result_rectangles = rectangles[:]
    number_of_rects = len(result_rectangles)
    threshold = 0.3
    cur_rect = 0
    while cur_rect < number_of_rects - 1:
        rects_to_compare = number_of_rects - cur_rect - 1
        cur_rect_to_compare = cur_rect + 1
        while rects_to_compare > 0:
            if IoU(result_rectangles[cur_rect], result_rectangles[cur_rect_to_compare]) >= 0.2 \
                    or IoM(result_rectangles[cur_rect], result_rectangles[cur_rect_to_compare]) >= threshold:
                del result_rectangles[cur_rect_to_compare]
                number_of_rects -= 1
            else:
                cur_rect_to_compare += 1
            rects_to_compare -= 1
        cur_rect += 1
    return result_rectangles


def detect_face_12c_net(net_12c, img_data, small_face_size,
                                 stride, multiScale = False, scale_factor = 1.414, threshold=0.05):
    net_kind = 12
    rectangles = []
    current_scale = find_initial_scale(net_kind, small_face_size)
    caffe_img_resized = resize_image(img_data, current_scale)
    current_height, current_width, channels = caffe_img_resized.shape

    while current_height > net_kind  and current_width > net_kind:
        caffe_img_resized_CHW = caffe_img_resized.transpose((2, 0, 1))
        net_12c.blobs['data'].reshape(1, *caffe_img_resized_CHW.shape)
        net_12c.blobs['data'].data[...] = caffe_img_resized_CHW
        net_12c.forward()
        out = net_12c.blobs['prob'].data[0][1, :, :]
        out_height, out_width = out.shape

        for current_y in range(0, out_height):
            for current_x in range(0, out_width):
                confidence = out[current_y, current_x]
                if confidence >= threshold:
                    current_rectangle = [int(2 * current_x * current_scale),
                                         int(2 * current_y * current_scale),
                                         int(2 * current_x * current_scale + net_kind * current_scale),
                                         int(2 * current_y * current_scale + net_kind *current_scale),
                                         confidence, current_scale]
                    rectangles.append(current_rectangle)
        if multiScale is False:
            break
        else:
            caffe_img_resized = resize_image(caffe_img_resized, scale_factor)
            current_scale *= scale_factor
            current_height, current_width, channels = caffe_img_resized.shape

    return rectangles





def cal_face_12c_net(net_12_cal, img_data, rectangles):
    height, width, channels = img_data.shape
    result = []
    for current_rectangle in rectangles:
        original_x1 = current_rectangle[0]
        original_y1 = current_rectangle[1]
        original_x2 = current_rectangle[2]
        original_y2 = current_rectangle[3]
        original_w = original_x2 - original_x1
        original_h = original_y2 - original_y1
        cropped_caffe_img = img_data[original_y1:original_y2, original_x1:original_x2]
        caffe_img_resized = cv2.resize(cropped_caffe_img, (12,12))
        caffe_img_resized_CHW = caffe_img_resized.transpose((2, 0, 1))
        net_12_cal.blobs['data'].reshape(1, *caffe_img_resized_CHW.shape)
        net_12_cal.blobs['data'].data[...] = caffe_img_resized_CHW
        net_12_cal.forward()

        output = net_12_cal.blobs['prob'].data

        prediction = output[0]

        threshold = 0.1
        indices = np. nonzero(prediction > threshold)[0]
        number_of_cals = len(indices)

        if number_of_cals == 0:
            result.append(current_rectangle)
            continue

        total_s_change = 0
        total_x_change = 0
        total_y_change = 0

        for current_cal in range(number_of_cals):
            cal_label = int(indices[current_cal])
            if (cal_label >= 0) and (cal_label <= 8):
                total_s_change += 0.83
            elif (cal_label >= 9) and (cal_label <= 17):
                total_s_change += 0.91
            elif (cal_label >= 18) and (cal_label <= 26):
                total_s_change += 1.0
            elif (cal_label >= 27) and (cal_label <= 35):
                total_s_change += 1.10
            else:
                total_s_change += 1.21

            if cal_label % 9 <= 2:  # decide x change
                total_x_change += -0.17
            elif (cal_label % 9 >= 6) and (
                    cal_label % 9 <= 8):  # ignore case when 3<=x<=5, since adding 0 doesn't change
                total_x_change += 0.17

            if cal_label % 3 == 0:  # decide y change
                total_y_change += -0.17
            elif cal_label % 3 == 2:  # ignore case when 1, since adding 0 doesn't change
                total_y_change += 0.17

        s_change = total_s_change / number_of_cals
        x_change = total_x_change / number_of_cals
        y_change = total_y_change / number_of_cals

        current_result = current_rectangle
        current_result[0] = int(max(0, original_x1 - original_w * x_change / s_change))
        current_result[1] = int(max(0, original_y1 - original_h * y_change / s_change))
        current_result[2] = int(min(width, current_result[0] + original_w / s_change))
        current_result[3] = int(min(height, current_result[1] + original_h / s_change))

        result.append(current_result)
    result = sorted(result, key = itemgetter(4), reverse = True)
    return  result

def  detect_face_24c_net(net_24c, img_data, rectangles):
    result = []
    for cur_rectangle in rectangles:
        x1 = cur_rectangle[0]
        y1 = cur_rectangle[1]
        x2 = cur_rectangle[2]
        y2 = cur_rectangle[3]

        cropped_caffe_img = img_data[y1:y2, x1:x2]
        caffe_img_resized = cv2.resize(cropped_caffe_img, (24, 24))
        caffe_img_resized_CHW = caffe_img_resized.transpose((2, 0, 1))
        net_24c.blobs['data'].reshape(1, *caffe_img_resized_CHW.shape)
        net_24c.blobs['data'].data[...] = caffe_img_resized_CHW
        net_24c.forward()

        prediction = net_24c.blobs['prob'].data
        confidence = prediction[0][1]

        if confidence > 0.05:
            cur_rectangle[4] = confidence
            result.append(cur_rectangle)
    return result



def cal_face_24c_net(net_24_cal, img_data, rectangles):
    height, width, channels = img_data.shape
    result = []
    for cur_rectangle in rectangles :
        original_x1 = cur_rectangle[0]
        original_y1 = cur_rectangle[1]
        original_x2 = cur_rectangle[2]
        original_y2 = cur_rectangle[3]
        original_w = original_x2 - original_x1
        original_h = original_y2 - original_y1
        cropped_caffe_img = img_data[original_y1:original_y2, original_x1:original_x2]
        caffe_img_resized = cv2.resize(cropped_caffe_img, (24, 24))
        caffe_img_resized_CHW = caffe_img_resized.transpose((2, 0, 1))
        net_24_cal.blobs['data'].reshape(1, *caffe_img_resized_CHW.shape)
        net_24_cal.blobs['data'].data[...] = caffe_img_resized_CHW
        net_24_cal.forward()

        output = net_24_cal.blobs['prob'].data
        prediction = output[0]
        threshold = 0.1
        indices = np.nonzero(prediction > threshold)[0]
        number_of_cals = len(indices)

        if number_of_cals == 0:
            result.append(cur_rectangle)
            continue
        total_s_change = 0
        total_x_change = 0
        total_y_change = 0

        for current_cal in range(number_of_cals):  # accumulate changes, and calculate average
            cal_label = int(indices[current_cal])  # should be number in 0~44
            if (cal_label >= 0) and (cal_label <= 8):  # decide s change
                total_s_change += 0.83
            elif (cal_label >= 9) and (cal_label <= 17):
                total_s_change += 0.91
            elif (cal_label >= 18) and (cal_label <= 26):
                total_s_change += 1.0
            elif (cal_label >= 27) and (cal_label <= 35):
                total_s_change += 1.10
            else:
                total_s_change += 1.21

            if cal_label % 9 <= 2:  # decide x change
                total_x_change += -0.17
            elif (cal_label % 9 >= 6) and (
                    cal_label % 9 <= 8):  # ignore case when 3<=x<=5, since adding 0 doesn't change
                total_x_change += 0.17

            if cal_label % 3 == 0:  # decide y change
                total_y_change += -0.17
            elif cal_label % 3 == 2:  # ignore case when 1, since adding 0 doesn't change
                total_y_change += 0.17

        s_change = total_s_change / number_of_cals
        x_change = total_x_change / number_of_cals
        y_change = total_y_change / number_of_cals

        cur_result = cur_rectangle
        cur_result[0] = int(max(0, original_x1 - original_w * x_change / s_change))
        cur_result[1] = int(max(0, original_y1 - original_h * y_change / s_change))
        cur_result[2] = int(min(width, cur_result[0] + original_w / s_change))
        cur_result[3] = int(min(height, cur_result[1] + original_h / s_change))

        result.append(cur_result)

    return result


def detect_face_48c_net(net_48c, img_data, rectangles):
    result = []
    for cur_rectangle in rectangles:
        x1 = cur_rectangle[0]
        y1 = cur_rectangle[1]
        x2 = cur_rectangle[2]
        y2 = cur_rectangle[3]

        cropped_caffe_img = img_data[y1:y2, x1:x2]
        caffe_img_sized = cv2.resize(cropped_caffe_img, (48, 48))
        caffe_img_sized_CHW = caffe_img_sized.transpose((2, 0, 1))
        net_48c.blobs['data'].reshape(1, *caffe_img_sized_CHW.shape)
        net_48c.blobs['data'].data[...] = caffe_img_sized_CHW
        net_48c.forward()
        prediction = net_48c.blobs['prob'].data
        confidence = prediction[0][1]

        if confidence > 0.1:
            cur_rectangle[4] = confidence
            result.append(cur_rectangle)
    result = sorted(result, key=itemgetter(4), reverse=True)
    return result


def cal_face_48c_net(net_48_cal, img_data, rectangles):
    height, width, channels = img_data.shape
    result = []
    for cur_rectangle in rectangles:
        original_x1 = cur_rectangle[0]
        original_y1 = cur_rectangle[1]
        original_x2 = cur_rectangle[2]
        original_y2 = cur_rectangle[3]
        original_w = original_x2 - original_x1
        original_h = original_y2 - original_y1

        cropped_caffe_img = img_data[original_y1:original_y2, original_x1:original_x2]
        caffe_img_resized = cv2.resize(cropped_caffe_img, (48, 48))
        caffe_img_resized_CHW = caffe_img_resized.transpose((2, 0, 1))
        net_48_cal.blobs['data'].reshape(1, *caffe_img_resized_CHW.shape)
        net_48_cal.blobs['data'].data[...] = caffe_img_resized_CHW
        net_48_cal.forward()
        output = net_48_cal.blobs['prob'].data

        prediction = output[0]

        threshold = 0.1
        indices = np.nonzero(prediction > threshold)[0]
        number_of_cals = len(indices)
        if number_of_cals ==0:
            result.append(cur_rectangle)
            continue
        total_s_change = 0
        total_x_change = 0
        total_y_change = 0

        for current_cal in range(number_of_cals):  # accumulate changes, and calculate average
            cal_label = int(indices[current_cal])  # should be number in 0~44
            if (cal_label >= 0) and (cal_label <= 8):  # decide s change
                total_s_change += 0.83
            elif (cal_label >= 9) and (cal_label <= 17):
                total_s_change += 0.91
            elif (cal_label >= 18) and (cal_label <= 26):
                total_s_change += 1.0
            elif (cal_label >= 27) and (cal_label <= 35):
                total_s_change += 1.10
            else:
                total_s_change += 1.21

            if cal_label % 9 <= 2:  # decide x change
                total_x_change += -0.17
            elif (cal_label % 9 >= 6) and (cal_label % 9 <= 8):  # ignore case when 3<=x<=5, since adding 0 doesn't change
                total_x_change += 0.17

            if cal_label % 3 == 0:  # decide y change
                total_y_change += -0.17
            elif cal_label % 3 == 2:  # ignore case when 1, since adding 0 doesn't change
                total_y_change += 0.17

        s_change = total_s_change / number_of_cals
        x_change = total_x_change / number_of_cals
        y_change = total_y_change / number_of_cals

        cur_result = cur_rectangle
        cur_result[0] = int(max(0, original_x1 - original_w * x_change / s_change))
        cur_result[1] = int(max(0, original_y1 - 1.1 * original_h * y_change / s_change))
        cur_result[2] = int(min(width, cur_result[0] + original_w / s_change))
        cur_result[3] = int(min(height, cur_result[1] + 1.1 * original_h / s_change))

        result.append(cur_result)
    return result



def detect_faces_net(nets, img_data, small_face_size, stride,
                     multiScale = False, scale_factor = 1.414, threshold=0.05):
    net_12c = nets[0]
    net_12_cal = nets[1]
    net_24c = nets[2]
    net_24_cal = nets[3]
    net_48c = nets[4]
    net_48_cal = nets[5]
    rectangles = detect_face_12c_net(net_12c, img_data, small_face_size,
                                     stride, multiScale, scale_factor, threshold)
    rectangles = cal_face_12c_net(net_12_cal, img_data, rectangles)
    rectangles = localNMS(rectangles)
    rectangles = detect_face_24c_net(net_24c, img_data, rectangles)
    rectangles = cal_face_24c_net(net_24_cal, img_data, rectangles)
    rectangles = localNMS(rectangles)
    rectangles = detect_face_48c_net(net_48c, img_data, rectangles)
    rectangles = globalNMS(rectangles)
    rectangles = cal_face_48c_net(net_48_cal, img_data, rectangles)
    rectangles = globalNMS_specify(rectangles)

    return rectangles